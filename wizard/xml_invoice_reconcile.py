...
class XMLInvoiceReconcile(models.TransientModel):
    ...
    forma_pago_id = fields.Many2one('payment.type', string='Forma de pago')
    methodo_pago = fields.Selection(
        selection=[('PUE', _('Pago en una sola exhibición')),
                   ('PPD', _('Pago en parcialidades o diferido')),],
        string=_('Método de pago'), 
    )
    uso_cfdi_id = fields.Many2one('uso.cfdi', string='Uso CFDI (cliente)')
    ...
    def action_reconcile(self):
        ...
        if invoice:
            ...
            invoice.write({'cfdi_folio_fiscal': self.folio_fiscal,
                           'payment_type_id' : self.forma_pago_id,
                           'payment_methodo' : self.methodo_pago,
                           'usocfdi_id' : self.uso_cfdi_id,
                           'cfdi_no_cetificado' : self.numero_cetificado,
                           'cfdi_fecha_timbrado': self.fecha_certificacion,
                           'invoice_date' : self.fecha_factura,
                           'cfdi_sello' : self.selo_digital_cdfi,
                           'sello' : self.selo_sat,
...
#            _logger.info("Factura conciliada")
        ...
        if payment:
            ...
            payment.write({'cfdi_folio_fiscal': self.folio_fiscal,
