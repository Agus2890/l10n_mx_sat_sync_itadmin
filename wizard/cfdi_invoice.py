...
class CfdiInvoiceAttachment(models.TransientModel):
    ...
    def import_customer_invoice(self, file_content, journal=False):
        ...
        invoice_vals = {
            ...
            'amount_total': data.get('Comprobante', {}).get('@Total'),
        }

        if relacionado_data:
           invoice_vals.update({})
           uuids_list = data.get('Comprobante', {}).get('CfdiRelacionados', {}).get('CfdiRelacionado', {})
           if type(uuids_list) != list:
               uuids_list = [uuids_list]
           uuids_relacionados = ''
           for relacionado in uuids_list:
               uuids_relacionados += relacionado.get('@UUID')
           invoice_vals.update({'tipo_relacion': relacionado_data.get('@TipoRelacion'), 'uuid_relacionado': uuids_relacionados})
    ...
    def import_supplier_credit_note(self, file_content, journal=False):
        ...
        if purchase_uuid != '':
            purchase_order_exist = invoice_obj.search([('cfdi_folio_fiscal', '=', purchase_uuid.lower())], limit=1)
            if not purchase_order_exist:
                purchase_order_exist = invoice_obj.search([('cfdi_folio_fiscal', '=', purchase_uuid.upper())], limit=1)
    ...
    @api.model
    def get_or_create_product(self, default_code, product_name, clave_unidad, unit_price, clave_producto, sale_ok=True,
                              purchase_ok=False):
        product_exist = False
        product_obj = self.env['product.product']
        param_obj = self.env['ir.config_parameter'].sudo()
        buscar_producto_por_clave_sat = self.env['ir.config_parameter'].sudo().get_param(
            'l10n_mx_sat_sync_itadmin.buscar_producto_por_clave_sat')
        product_types = dict(product_obj._fields.get('type')._description_selection(product_obj.env))
        product_type_default = self.env['ir.config_parameter'].sudo().get_param('l10n_mx_sat_sync_itadmin.product_type_default')
    ...
    def import_purchase_order(self, file_content):
        ...
        purchase_uuid = timbrado_data.get('@UUID')
        if purchase_uuid != '':
            purchase_order_exist = purchase_obj.search([('cfdi_folio_fiscal', '=', purchase_uuid.lower())], limit=1)
            if not purchase_order_exist:
                purchase_order_exist = purchase_obj.search([('cfdi_folio_fiscal', '=', purchase_uuid.upper())], limit=1)
            if purchase_order_exist:
                raise UserError("Factura ya existente con ese UUID %s" % (purchase_uuid))
