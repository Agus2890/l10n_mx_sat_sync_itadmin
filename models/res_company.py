                    ...
                    if customer_documents:
                        if cfdi_type == 'P':
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                payment_exist = payment_obj.search([('cfdi_folio_fiscal', '=', uu), ('company_id','=',self.id)], limit=1)
                                if payment_exist:
                                    vals.update({'creado_en_odoo': True, 'payment_ids': [(6, 0, payment_exist.ids)]})
                                    break
                        elif cfdi_type == 'E':
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                invoice_exist = invoice_obj.search([('cfdi_folio_fiscal', '=', uu), ('company_id','=',self.id)], limit=1)
                                if invoice_exist:
                        ...
                        if cfdi_type == 'SP':
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                payment_exist = payment_obj.search(
                                    [('cfdi_folio_fiscal', '=', uu), ('payment_type', '=', 'outbound'), ('company_id','=',self.id)], limit=1)
                                if payment_exist:
                                    vals.update({'creado_en_odoo': True, 'payment_ids': [(6, 0, payment_exist.ids)]})
                                    break
                        elif cfdi_type == 'SE':
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                invoice_exist = invoice_obj.search(
                                    [('cfdi_folio_fiscal', '=', uu), ('move_type', '=', 'in_refund'), ('company_id','=',self.id)], limit=1)
                                if invoice_exist:
                                    vals.update({'creado_en_odoo': True, 'invoice_ids': [(6, 0, invoice_exist.ids)]})
                                    break
                        else:
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                invoice_exist = invoice_obj.search(
                                    [('cfdi_folio_fiscal', '=', uu), ('move_type', '=', 'in_invoice'), ('company_id','=',self.id)], limit=1)
                                if invoice_exist:
                                    vals.update({'creado_en_odoo': True, 'invoice_ids': [(6, 0, invoice_exist.ids)]})
                                    break
    ...
    def download_cfdi_invoices_web(self, start_date=False, end_Date=False):
        ...
        if invoice_content_receptor:
            ...
            for uuid,data in invoice_content_receptor.items():
                ...
                if cfdi_type=='SP':
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        payment_exist = payment_obj.search([('cfdi_folio_fiscal','=',uu), ('payment_type','=','outbound'), ('company_id','=',self.id)],limit=1)
                        if payment_exist:
                            vals.update({'creado_en_odoo' : True,'payment_ids':[(6,0, payment_exist.ids)]})
                            break
                if cfdi_type=='SE':
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('cfdi_folio_fiscal','=',uu), ('move_type','=','in_refund'), ('company_id','=',self.id)],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'invoice_ids':[(6,0, invoice_exist.ids)]})
                            break
                else:
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('cfdi_folio_fiscal','=',uu), ('move_type','=','in_invoice'), ('company_id','=',self.id)],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'invoice_ids':[(6,0, invoice_exist.ids)]})
                            break
                attachment_obj.create(vals)
